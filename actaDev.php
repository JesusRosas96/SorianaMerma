<?php
    ...
    $valor2=mysqli_query($conn, "SELECT COUNT(*) AS cant FROM acta;");
                ...
                <div class="container-fluid buscador">
                    <div class="row">
                        ...
                        <div id="formalities" class="row">
                            <?php
                                $valor=mysqli_query($conn, "SELECT * FROM acta WHERE estado= 0;");
                                    while($row= mysqli_fetch_array($valor)){
                                    echo '<div class="col">
                                            <a href="formato.php"></a>
                                            <div class="card">';
                                    $_SESSION['Folio']= $row['folio'];
                                    echo   '<a href="formato.php">
                                            <div class="card-body">
                                                <p class="card-text">Folio: '.$row['folio'].' Código: '.$row['codigoPro'].'<br>Área: '.$row['area'].' Proveedor: '.$row['proveedor'].'<br>Fecha de emisión: '.date("d/m/Y",strtotime($row['fechaEm'])).'<br>Fecha de destrucción: '.date("d/m/Y",strtotime($row['fechaEm']."+ 7 days")).'</p>
                                            </div>
                                            </a>
                                            </div>
                                            </div>';
                                    ...
                                    if(date("d/m/Y",strtotime($row['fechaEm']."+ 7 days")) == date("d/m/Y")){
                                        $valor3=mysqli_query($conn, "UPDATE FROM acta SET estado= 1;");
                                    }
